<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:7F00FF,100:00D4FF&height=180&section=header&text=Hi%20there!%20I'm%20Cedric%20👋&fontSize=40&fontColor=ffffff&animation=fadeIn" />

<p>
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=900&color=00D4FF&center=true&vCenter=true&width=520&lines=Computer+Science+Student+%F0%9F%8E%93;Building+cool+things+with+code+%F0%9F%9A%80;Always+learning+%26+shipping+%E2%9C%A8" />
</p>

<p>
  <a href="https://www.linkedin.com/in/cedric-chung-2756b4310/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:cedricctf11a@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

</div>

---

## ✨ About me
- 🎯 Currently: **Final Year CS Student** focusing on **AI-driven Blockchain solutions** and scalable **Full-Stack applications**.
- 💡 Interests: **Web3 Architecture / Generative AI / DeFi Protocol Design / System Security**
- 🌱 Learning: **Advanced Smart Contract Optimization (Solidity/Sui Move) & LLM Orchestration**
- ⚡ Fun fact: **I’ve spent more time in hackathons than in some of my actual lecture halls!** 🚀

---

## 🧰 Tech Stack
<div align="center">

<img src="https://skillicons.dev/icons?i=python,java,js,ts,solidity,go,rust,react,nextjs,nodejs,express,html,css,tailwind,git,github,linux,docker,mysql,postgres,mongodb,vercel&perline=11" />

</div>

---

## 🏆 Hackathon Achievements

| Hackathon | Achievement | Project & Description | Links |
| :--- | :--- | :--- | :--- |
| **FinHack 2026** | 🏅 **4th Place** | **RoadBlock**: An AI-powered transaction intelligence layer for TNG eWallet that transforms transaction data into real-time intent signals, proactively surfacing the right miniapp vouchers, cashback offers, and promotions at the optimal moment—either during payment for impulse-driven decisions or after payment for higher-consideration purchases. | [Repo](https://github.com/Cedctf/roadblock) |
| **Monad Blitz KL 2026** | 🥈 **2nd Place** | **Brain Hotel**: A knowledge-preservation platform that transforms expert experiences, memories, strategies, and lessons into AI-powered memory vaults. Users can explore and query human expertise through an interactive hotel-style interface, preserving valuable knowledge long after experts leave a field. | [Repo](https://github.com/Cedctf/brainhotel) |
| **ETHGlobal New York 2026** | 🥇 **1st (Arc)**, 🥇 **1st (Hedera)**, 🥈 **2nd (World ID)** | **MARS**: Marketplace for Audited, Reputable Skills—an app store for AI-agent skills where World-ID-verified auditors compete to vet MCP tools and agent capabilities, Chainlink attests verdicts, Hedera records trust scores and verification, and agents pay-per-version via Arc x402 to safely use only audited skills. | [Repo](https://github.com/Cedctf/ethnyc) |
| **ETHGlobal Cannes 2026** | 🏆 **Finalist**, 🥇 **World Best use Of MiniKit 2.0 (1st)**, 🥇 **AI & Agentic Payments on Hedera (1st)**, 🥈**Best OpenClaw Agent on 0G (2nd)**| **DIVE**: A decentralized intelligence verification engine where human-backed AI swarms resolve real-world truth and trigger on-chain outcomes for prediction markets and autonomous settlement. | [Repo](https://github.com/Cedctf/cannes2026) |
| **First Movers Sprint 2026** | 🥇 **1st (DeFi)** | **Phởcast**: A multi-dimensional prediction market using a Joint-Outcome AMM to unify liquidity across correlated binary events. | [Repo](https://github.com/Cedctf/six-seven) |
| **Cursor x Anthropic 2025** | 🎯 **Top 10 Finalist** | **EternAI**: A digital legacy platform that uses RAG and fine-tuned LLMs to create interactive, memory-mapped AI avatars of loved ones. | [Repo](https://github.com/Cedctf/eternai) |
| **Forte Hacks by Flow 2025** | 🥉 **3rd (Existing Code)** | **DhalWay**: A universal cross-chain payment protocol using LayerZero OFTs and EIP-712 for one-signature merchant settlement. | [Repo](https://github.com/Cedctf/dhal-way) |
| **ETHOnline 2025** | 🥇 **1st (BlockScout)** | **PetPet**: A pet-centric explorer using IoT (ESP32) and TEEs (Phala) to create privacy-safe, verifiable on-chain pet activity logs. | [Repo](https://github.com/Cedctf/pet-pet) |
| **Token2049 Singapore 2025** | 🥇 **1st (Celo), Top 5 Finalist 🏆** | **LeftAI**: A natural-language MiniPay MiniApp for Celo that simplifies crypto transfers, bill splitting, and staking via AI. | [Repo](https://github.com/Cedctf/token2049) |
| **ETHGlobal New Delhi 2025** | 🥇 **1st (Flow), Hedera (Best Overall Project)** | **DhalWay**: Cross-chain intent-based payments allowing users to split checkouts across any token/chain via a single signature. | [Repo](https://github.com/Cedctf/ethindia) |
| **ETHTokyo 2025** | 🥇 **AI Track Champion, Finalist🏆** | **Toku Kaigan**: Privacy-first AI psychiatrist using TEEs, Swarm Intelligence, and manga-style progress tracking. | [Repo](https://github.com/Cedctf/ethtokyo) |
| **DevQuest 2025** | 🥇 **Champion** | **Competitive Debugging**: Ranked 1st in a live sprint, resolving critical bugs and implementing features on legacy codebases. | N/A |
| **Devmatch 2025** | 🥇 **Best Tooling For Prediction Markets, 🥈 2nd (Best Development Of Subgraph)** | **Ethclbet**: A prediction market aggregator that uses Oasis ROFL/Sapphire and subgraphs to privately optimize and execute cross-platform bets. | [Repo](https://github.com/Cedctf/ethclbet) |
| **IOTA Malaysia 2025** | 🎯 **Top 10** | **Cap Attendance**: On-chain identity (DID) and face-recognition system for tamper-proof academic attendance. | [Repo](https://github.com/Cedctf/iota-hackathon) |
| **EasyA x Ripple Singapore 2025** | 🥈 **2nd Place** | **Wiser**: Self-custodial crypto debit card infra using XRPL pathfinding and Marqeta JIT funding. | [Repo](https://github.com/Cedctf/EasyA-Wiser) |
| **SuperteamMY Mega Hackathon** | 🎯 **8th Place** | **Wiser (Solana)**: Universal payment middleware linking Visa rails to Solana for bi-directional fiat-crypto transactions. | [Repo](https://github.com/Cedctf/EasyA-Wiser) |
| **ImagineHack 2025** | 🥇 **Champion** | **Tea Time**: AI-powered PWA fighting misinformation via sentiment analysis and crowdsourced bias checks. | [Repo](https://github.com/Cedctf/tt) |
| **ETHGlobal Agentic 2025** | 🥈 **2nd (Flow)** | **4AI 1Human**: A multi-agent system that deploys and manages custom smart contracts via plain-English commands. | [Repo](https://github.com/Cedctf/agentic) |
| **PayHack 2024** | 🥉 **3rd Place** | **Finpet**: A gamified financial literacy app using an evolving virtual pet to incentivize healthy saving habits. | [Repo](https://github.com/Cedctf/payhack) |

---

## 📈 GitHub Stats
<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Cedctf&show_icons=true&rank_icon=github&theme=tokyonight&hide_border=true&cache_seconds=21600" />
<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Cedctf&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" />

</div>

<div align="center">
  <img src="https://raw.githubusercontent.com/Cedctf/Cedctf/output/github-contribution-grid-snake.svg" />
</div>

---

## 🤝 Let’s connect
- 💼 LinkedIn: [linkedin.com/in/cedric-chung-2756b4310](https://www.linkedin.com/in/cedric-chung-2756b4310/)
- 📫 Email: cedricctf11a@gmail.com

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00D4FF,100:7F00FF&height=120&section=footer" />
</div>
